...
* Security hardening release. NGINX now answers **403** for anonymous requests that would reveal the site's login names: the `wp/v2/users` REST route in both of its spellings (`/wp-json/wp/v2/users` and the `/?rest_route=/wp/v2/users` fallback), and the author archive addressed by numeric id (`/?author=1`, which redirects to `/author/<login>/`). Matching runs on the normalized path, so percent-encoding, duplicate slashes and `.` segments do not get around it. Implemented as a map plus a rewrite-phase guard, independent of location matching order — the same mechanism as the PHP execution allowlist.
* Requests carrying a WordPress session cookie are exempt, so the block editor's author selector and the wp-admin posts-list author filter keep working. The cookie is checked for presence, not validated: this separates automated scanners from logged-in browsers, and is not an authorization boundary against a targeted attacker. The pretty `/author/<slug>/` archive is a public feature and stays reachable.
* New values: `restApiHardening.mode` (`enforce` [default] | `report` | `off`) and `restApiHardening.extraDeniedPaths` for denying further requests. `report` mode logs would-be blocks as `[rest-hardening]` lines on container stderr without blocking.
* Extended the smoke test to cover every route spelling, the encoding variants, numeric and non-numeric author arguments, the session-cookie exemption, the extra-deny patterns and all three modes.
