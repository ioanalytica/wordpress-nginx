...
* **`smtpProtocol` has real semantics now**: `tls` = STARTTLS (usually port 587), `ssl` = implicit TLS (smtps, port 465), empty = unencrypted (internal relays only). Any other value fails the render instead of silently sending unencrypted. Sites that had `smtpProtocol` set gain encryption on the first rollout of this release - until now the value did nothing and msmtp spoke plaintext. If your relay's certificate does not validate against the system CA bundle, sending fails loudly from this release on; fix the certificate rather than the setting.
* Implicit TLS needed image support: the sendmail wrapper only exposed msmtp's `--tls` switch, which upgrades via STARTTLS. The wrapper now understands `SMTP_STARTTLS` (the chart sets it to `off` for `smtpProtocol: ssl`), passed to msmtp as `--tls-starttls`. A build-time guard fails the image build if the base image rewrites the wrapper and the patch no longer lands.
* **`smtpFromEmail` is the sender, envelope AND header.** It becomes msmtp's envelope sender (`--from`), and the message's From *header* is rewritten to match on the way into msmtp - transport-side canonical-sender rewriting (the job Postfix does with `sender_canonical`/`header_checks`), performed by the new `rewrite-mail-from` step in the image's sendmail wrapper. This matters beyond cosmetics: submission services tie the sender to the authenticated identity - Office 365 rejects a mismatched header From with `SendAsDenied`, and WordPress writes `wordpress@<site host>` there unless site code filters it. No code is injected into WordPress; the whole mail configuration stays in the environment. Unset, envelope and header leave the pod exactly as WordPress wrote them, as before. The rewrite replaces only the first From header of the header block (folded continuations included), inserts one if the message has none, and never touches the body.
* **`smtpFromName` sets the human-readable display name** of the rewritten From header (`"Name" <smtpFromEmail>`); non-ASCII names are RFC 2047 encoded. It requires `smtpFromEmail` and fails the render when set alone - a display name without an address configures nothing, and a value that silently does nothing is exactly how this block was broken before.
* The smoke test pins the wrapper's option composition and the header rewrite (hermetically, against a stub msmtp): TLS switches, envelope sender, auth toggle, the password traveling via `--passwordeval` and never via argv, the loud refusal without `SMTP_HOST`, From replacement including folded headers and insert-when-missing, RFC 2047 encoding, the body staying untouched, and byte-identical passthrough when `smtpFromEmail` is unset. The template test pins the value-to-variable mapping, that unknown `smtpProtocol` values and a standalone `smtpFromName` fail the render, and that the SMTP password reaches the pod only through a Secret.
...
* The W3 Total Cache configuration script now declares `SERVER_SOFTWARE=nginx`. Under wp-cli there is no server signature, so W3TC fell back to managing Apache `.htaccess` rules; the WordPress root is immutable here, and every boot from the second one on logged W3TC's misleading `FTP credentials don't allow to write to file /var/www/html/.htaccess` error (harmless — NGINX never reads `.htaccess`, and the cache options were all applied regardless). With the server declared, `fix_environment` maintains its rules in the plugin-managed `nginx.conf` instead.
