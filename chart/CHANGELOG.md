...
* **`smtpFromEmail` is the envelope sender** (msmtp `--from`). Set it when the relay requires the envelope to match the authenticated identity. Unset, the envelope is read from the message - WordPress' own From header, `wordpress@<site host>`, as before. Deliberately transport-only: the visible From header stays under WordPress' control (`wp_mail_from` filter).
* **`smtpFromName` was removed** and fails the render when set. A From display name is not transport configuration, and the chart does not inject code into WordPress to set one - use the `wp_mail_from_name` filter in the site. The value never had any effect, so removing it changes no behavior; the loud failure is there because a value that silently does nothing is exactly how this block was broken before.
* The smoke test pins the wrapper's option composition (hermetically, against a stub msmtp): TLS switches, envelope sender, auth toggle, the password traveling via `--passwordeval` and never via argv, and the loud refusal without `SMTP_HOST`. The template test pins the value-to-variable mapping, that unknown `smtpProtocol` values and the removed `smtpFromName` fail the render, and that the SMTP password reaches the pod only through a Secret.
